//************* For Endcap ******************//
//! plot pull distribution for various eta bins
...
//! sigma(t) = uncertainty in transverse(azimuthal) direction = w 1e-3 / 12^0.5 
//! w [μm] = Pixel width (varies as a function of radius) = r02*pixel_maxwidth/r_max = r02*50*e-3/758
//! d = TTT endcap radial gap-size (changes as a function of eta)
//! t/X = 1.5% = 0.015
//! B = 4T
#include <iostream>
#include "TROOT.h"
...
const double r_max    	= 758;//mm
const double w_max    	= 50e-3;//mm
const double z_gap	= 80;// gap between the layers in the triplet in mm at eta = 1.7
const double B		= 4;// magnetic field in Tesla
const double X_0	= 0.015;// relative radiation length t/X_0  = 2%
double w = 0, d = 0; // pixel width and radial gap-size to be calculated on the fly

const char* out_path = "./plots";

//! 80mm
TCut cut = "Tid>0 && Pt_n>2e3 && abs(Z013)<100 && abs(Eta13)>1.7 && abs(Phi13)<0.021 && abs(Eta3 - Eta1)<0.006 && abs(dphi2)<3.0e-3 && abs(Eta2-0.5*(Eta1+Eta3))<0.002";
TCut MS_r = "Tid>0 && Pt_n>2e3 && Pt_n<4e3 && abs(Z013)<100 && abs(Eta13)<1.7 && abs(Phi13)<0.021 && abs(Eta3 - Eta1)<0.006 && abs(dphi2)<3.0e-3 && abs(Eta2-0.5*(Eta1+Eta3))<0.002";
TCut Hit_r = "Tid>0 && Pt_n>20e3 && abs(Z013)<100 && abs(Eta13)<1.7 && abs(Phi13)<0.021 && abs(Eta3 - Eta1)<0.006 && abs(dphi2)<3.0e-3 && abs(Eta2-0.5*(Eta1+Eta3))<0.002";

int pull(const char* output_file_name = "pullggF_80mm", const char* constants ="B")
{
	TChain rec("m_recTree");
	rec.Add("/user/tkar/work/data/rec/for_opt/Br30mmEC80mm/ggF1.0/*.root");

	int n_events = rec.GetEntries();
	std::cout<<"rec tree entries: "<< n_events << std::endl;
	gStyle->SetOptFit(1011);
	
	std::vector<double> *r02 = 0;
	std::vector<double> *theta013 = 0;
	std::vector<double> *eta013 = 0;
	
	rec.SetBranchStatus("*", 0);
	rec.SetBranchStatus("R02", 1);
	rec.SetBranchStatus("Theta13", 1);
	rec.SetBranchStatus("Eta13", 1);
	
	rec.SetBranchAddress("R02", &r02);
	rec.SetBranchAddress("Theta13", &theta013);
	rec.SetBranchAddress("Eta13", &eta013);

	for (int i=0; i < n_events; i++)
	{
		rec.GetEntry(i);
		for (int j = 0; j < r02->size(); j++)
		{
			//! determine the width of Pixel using the hit on middle layer
			w = r02->at(j) * w_max / r_max; 
			//! determine radial gap-size (d) using the eta (or theta)
			d = z_gap *tan(theta013->at(j))
		}
	...
	double xmin = 1.7, xmax = 2.5;
	double xmin1,xmax1;
	//! Book histograms
	TH1F *h	  = new TH1F("h"  ,"Eta distribution; #eta_{013}",nbins,xmin,xmax);
	rec.Draw("abs(Eta13)>>h",cut,"",1e5);
	...
	*/
	...
	for(int i = 0; i<10; ++i)
	{
		...
		*/
		...
		sprintf(buffer,"abs(Eta13) > %f && abs(Eta13) < %f",xmin1,xmax1);
