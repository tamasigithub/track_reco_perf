...
int write_topdf(const char* output_file_name = "ResoVsEta_InDetTTTBMatched")
{
	...
	//!InDet tracks
	TFile* f = TFile::Open("./plots/reso_plots/ResoVsetaInDet_dR0.01Matched.root");
...
//	TH1D* h_dca	= (TH1D*)f->Get("h_sigma_dca");
	...
	//! TTT tracks
	TFile* f0 = TFile::Open("./plots/reso_plots/ResoVseta_barcodeMatched.root");
...
/*	h_dca->GetYaxis()->SetRangeUser(10,40);
	...
	h0_eta->SetLineColor(kBlue);
	h_eta->SetMarkerColor(kBlue);
	h0_eta->SetMarkerColor(kBlue);	
	h_eta->Draw();
	h0_eta->Draw("same");
	C->Update();
	Float_t rightmax = 1*h_phi->GetMaximum();
...
//	axis->SetLabelSize(0.05);
	axis->SetTitle("#sigma_{#Delta(#phi)}");
