...
void weighted_avgPurity(const char *pileup, const char *gapsize)
{
	...
	sprintf(signal_file_names, "%s/Br30mmEC%smm/PU%s/ggF1.0/*.root",file_path,gapsize,pileup);
...
void rmsVspurity(const char *run_no, const char *pileup, const char *gapsize)
{
	...
	TChain t("m_recTree");
	sprintf(rec_file_names, "%s/single_pi/pi*_recTree_%s.root",file_path,run_no);
	//sprintf(rec_file_names, "%s/Br30mmEC%smm/PU%s/ggF1.0/*.root",file_path,gapsize,pileup);
	t.Add(rec_file_names);
	std::cout<<"Number of entries in single pi files: " << t.GetEntries()<<std::endl;
...
int writePU0()
{
	...
	deleteText("0");	
	weighted_avgPurity("0", "53");
	rmsVspurity("530010","0","53");
	rmsZvtxVspurity("530010","0","53");
	
	weighted_avgPurity("0", "67");
	rmsVspurity("670010","0","67");
	rmsZvtxVspurity("670010","0","67");
	
	weighted_avgPurity("0", "80");
	rmsVspurity("800010","0","80");
	rmsZvtxVspurity("800010","0","80");
	
	weighted_avgPurity("0", "93");
	rmsVspurity("930010","0","93");
	rmsZvtxVspurity("930010","0","93");
	
	weighted_avgPurity("0", "106");
	rmsVspurity("106010","0","106");
	rmsZvtxVspurity("106010","0","106");
	return 0;
}
int writePU200()
{
	...
	deleteText("200");	
	weighted_avgPurity("200", "53");
	rmsVspurity("530010","200","53");
	rmsZvtxVspurity("530010","200","53");
	
	weighted_avgPurity("200", "67");
	rmsVspurity("670010","200","67");
	rmsZvtxVspurity("670010","200","67");
	
	weighted_avgPurity("200", "80");
	rmsVspurity("800010","200","80");
	rmsZvtxVspurity("800010","200","80");
	
	weighted_avgPurity("200", "93");
	rmsVspurity("930010","200","93");
	rmsZvtxVspurity("930010","200","93");
	
	weighted_avgPurity("200", "106");
	rmsVspurity("106010","200","106");
	rmsZvtxVspurity("106010","200","106");
	return 0;
}
int writePU1000()
{
	...
	deleteText("1k");	
	weighted_avgPurity("1k", "53");
	rmsVspurity("530010","1k","53");
	rmsZvtxVspurity("530010","1k","53");
	
	weighted_avgPurity("1k", "67");
	rmsVspurity("670010","1k","67");
	rmsZvtxVspurity("670010","1k","67");
	
	weighted_avgPurity("1k", "80");
	rmsVspurity("800010","1k","80");
	rmsZvtxVspurity("800010","1k","80");
	
	weighted_avgPurity("1k", "93");
	rmsVspurity("930010","1k","93");
	rmsZvtxVspurity("930010","1k","93");
	
	weighted_avgPurity("1k", "106");
	rmsVspurity("106010","1k","106");
	rmsZvtxVspurity("106010","1k","106");
	return 0;
}
...
void summary_relptVspurity()
{
	...
	g1->GetXaxis()->SetTitle(Form("Average track purity, p_{T} #in (%d, %d) GeV/c", (int)X_min, (int)X_max));
	g1->GetYaxis()->SetTitle("#sigma_{#delta p_{T}/p_{T, gen}} [%], at p_{T} = 10 GeV/c");
	...
	g3->GetYaxis()->SetRangeUser(0,2.5);
	g3->GetXaxis()->SetLimits(0.80,1);
...
/////////////////////////////////////////////////////////////
	...
	TLegend *leg=new TLegend(X1,Y2,X2,Y2+0.07,"      <#mu>      gap size");
	...
	TLegend *leg1=new TLegend(X1+0.01,Y1,X2+0.01,Y2,"1k 200   0");
	...
	if(j == n - 5) 
	{
		leg1->AddEntry(m1,"   ");
		leg1->AddEntry(m2,"   ");
		leg1->AddEntry(m3,"   53mm");
	}
	if(j == n - 4) 
	{
		leg1->AddEntry(m1,"   ");
		leg1->AddEntry(m2,"   ");
		leg1->AddEntry(m3,"   67mm");
	}
	if(j == n - 3) 
	{
		leg1->AddEntry(m1,"   ");
		leg1->AddEntry(m2,"   ");
		leg1->AddEntry(m3,"   80mm");
	}
	if(j == n - 2)
	{
		leg1->AddEntry(m1,"   ");
		leg1->AddEntry(m2,"   ");
		leg1->AddEntry(m3,"   93mm");
	}
	if(j == n -1)
	{
		leg1->AddEntry(m1,"   ");
		leg1->AddEntry(m2,"   ");
		leg1->AddEntry(m3,"  106mm");
	}
	}
	leg->Draw();
	leg1->Draw();
...
void summary_ZvtxVspurity()
{
	...
	g1->GetXaxis()->SetTitle(Form("Average track purity, p_{T} #in (%d, %d) GeV/c", (int)X_min, (int)X_max));
	g1->GetXaxis()->SetLimits(0.80,1);
	g1->GetYaxis()->SetTitle("#sigma_{#delta z }[mm], at p_{T} = 10 GeV/c");
	g1->GetYaxis()->SetRangeUser(0,2.0);
	g1->SetMarkerStyle(kFullCircle);
	g1->GetXaxis()->CenterTitle();
	g1->GetYaxis()->CenterTitle();
	g1->GetXaxis()->SetTitleOffset(AXISTITLE_OFFSET);
	...
	g3->GetYaxis()->SetRangeUser(0,2.0);
