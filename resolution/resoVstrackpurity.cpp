...
void write_file(const char* gapsize, const char* pileup, double purity , double rms, double error)
{
    ...
    const char* txt_path = "./txt_files";
    char txt_file[1023];
    sprintf(txt_file,"%s/summary20-100_rmsVspurityAllmm_PU%s.txt",txt_path, pileup);
    std::ofstream ofs;
...
void write_file_Zvtx(const char* gapsize, const char* pileup, double purity , double rms, double error)
{
    ...
    const char* txt_path = "./txt_files";
    char txt_file[1023];
    sprintf(txt_file,"%s/summary20-100_rmsZvtxVspurityAllmm_PU%s.txt",txt_path, pileup);
    std::ofstream ofs;
    ofs.open (txt_file, std::ofstream::out | std::ofstream::app);
    ofs<<gapsize<<" "<<purity<<" "<<rms<<" "<<error<<"\n";
    ofs.close();
...
void summary_relptVspurity()
{
	...
	//! PU 1000
	sprintf(txt_file1,"%s/summary20-100_rmsVspurityAllmm_PU%s.txt",txt_path,"1000");
	//! PU 200
	sprintf(txt_file2,"%s/summary20-100_rmsVspurityAllmm_PU%s.txt",txt_path,"200");
	...
	g1->GetXaxis()->SetTitle("Track Purity");
	g1->GetXaxis()->SetRangeUser(0.5,1);
	g1->GetYaxis()->SetTitleOffset(1.5);
	g1->GetYaxis()->SetTitle("#sigma_{#Delta(p_{t})/gen_p_{t}} @ p_{t} = 10 GeV/c");
	g1->GetYaxis()->SetRangeUser(0,0.015);
...
//	g1->SetTitle("<#mu> = 960");
	...
	g2->GetXaxis()->SetTitle("Track Purity");
	g2->GetXaxis()->SetRangeUser(0.5,1);
	g2->GetYaxis()->SetTitleOffset(1.5);
	g2->GetYaxis()->SetTitle("#sigma_{#Delta(p_{t})/gen_p_{t}} @ p_{t} = 10 GeV/c");
	g2->GetYaxis()->SetRangeUser(0,0.015);
...
/////////////////////////////////////////////////////////////
	...
	Double_t x1= 0.7,y1= 0.1, x2 = 0.9, y2 = 0.28;
	...
	leg1->Draw();
	c1->Update();
	SaveCanvas(c1,"relptVspurityAllmm_varPU");
...
void summary_ZvtxVspurity()
{
	...
	char plot_title[1023];
	//! PU 1000
	sprintf(txt_file1,"%s/summary20-100_rmsZvtxVspurityAllmm_PU%s.txt",txt_path,"1000");
	//! PU 200
	sprintf(txt_file2,"%s/summary20-100_rmsZvtxVspurityAllmm_PU%s.txt",txt_path,"200");
	//sprintf(txt_file3,"%s/summary20-100_rmsVspurity_PU%s.txt",txt_path,pileup);
...
/////////////////////////////////////////////////////////////
	...
	leg1->Draw();
	c1->Update();
	SaveCanvas(c1,"ZvtxVspurityAllmm_varPU");
