...
int ratio_theta(const char* output_file_name = "ratio_theta", bool debug = false)
{
	...
	TH1F* h1 = new TH1F("h1","reconstructed theta; theta[rad];",nbins, thetamin, thetamax);//ttt 
	TH1F* h2 = new TH1F("h2","h2",nbins, thetamin, thetamax);

	tree.Draw("TTTtheta>>h1");
	tree.Draw("InDettheta>>h2",InDet_cut);
	h1->GetYaxis()->SetRangeUser(0,80000);
