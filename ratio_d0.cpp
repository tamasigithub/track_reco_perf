...
TCut InDet_cut1 = "abs(sin(InDettheta)/InDetqOverP) > 2e3 && abs(sin(InDettheta)/InDetqOverP) < 10e3 && abs(-log(tan(InDettheta/2)))<1.5 && abs(InDetz0)<100";
TCut TTT_cut1 = "abs(sin(InDettheta)/InDetqOverP) > 2e3 && abs(sin(InDettheta)/InDetqOverP) < 10e3";
int ratio_d0(const char* output_file_name = "ratio_d0", bool debug = false)
{
	...
	TH1F* h1 = new TH1F("h1","reconstructed d0; d0[mm];",nbins, xmin, xmax);//ttt 
	TH1F* h2 = new TH1F("h2","reconstructed d0; d0[mm];",nbins, xmin, xmax);

	tree.Draw("TTTd0>>h1");
	tree.Draw("InDetd0>>h2",InDet_cut);
	//tree.Draw("TTTd0>>h1",TTT_cut1);
	//tree.Draw("InDetd0>>h2",InDet_cut1);
