...
const double dphi2_max = 1e-3;
const double deta2_max = 1e-3;
...
int eff_Vs_etaphipt_dR_barcode(const char* output_file_name = "PU1kallpcles_ggF1.0-eff_30mmEC80mm_sel5_2")
{
	...
	recTree.Add("/user/tkar/work/data/rec/sel/Br30mmEC80mm/PU1k/ggF1.0/user.tkar*.root");
	truthTree.Add("/user/tkar/work/data/rec/sel/Br30mmEC80mm/PU1k/ggF1.0/user.tkar*.root");
	...
	std::cout<<"truth_entries: " << truth_entries <<std::endl;
	std::cout<<"truth_entries: " << truthTree.GetEntries() <<std::endl;
	...
	std::vector<int>   *tinteraction = 0;
        ...
        std::vector<double> *Eta1 = 0;
        std::vector<double> *Eta2 = 0;
        std::vector<double> *Eta3 = 0;
        std::vector<double> *dphi2 = 0;
	...
	recTree.SetBranchStatus("station",  	1);
	recTree.SetBranchStatus("Tid",  	1);
	recTree.SetBranchStatus("kappa_pull",  	1);
	recTree.SetBranchStatus("Eta1",  	1);
	recTree.SetBranchStatus("Eta2",  	1);
	recTree.SetBranchStatus("Eta3",  	1);
	recTree.SetBranchStatus("dphi2",  	1);
	...
	recTree.SetBranchAddress("station", &station);
	recTree.SetBranchAddress("kappa_pull", &k_pull);
	recTree.SetBranchAddress("Eta1", &Eta1);
	recTree.SetBranchAddress("Eta2", &Eta2);
	recTree.SetBranchAddress("Eta3", &Eta3);
	recTree.SetBranchAddress("dphi2", &dphi2);
...
*/	/// LOG BINS for signal sample
	...
	for(int i = 0; i < truth_entries; ++i)
	{
		if(i > truth_entries) break;
		...
		for(int i1 = 0; i1 < barcode->size(); ++i1)
		{
			...
			if(std::abs(pdg->at(i1)) == 0) 		continue;
			...
			if(hypot( Vx->at(i1), Vy->at(i1) ) > 5 )		continue;
			...
			for(int i2 = 0; i2 < match_barcodeTTT_bc->size(); ++i2)
			{
				...
				if(std::fabs((*Eta2)[i2]-0.5*((*Eta1)[i2]+(*Eta3)[i2])) > deta2_max) continue;
			...
			if(match_flagTTT_bc > -1)
			{
				...
				else if(std::abs(pdg->at(i1)) == 11)
				{
					h_den_vs_etae->Fill((*eta)[i1]);
					h_den_vs_phie->Fill((*phi)[i1]);
					h_den_vs_pte->Fill((*pt)[i1]);
				}
				else if(std::abs(pdg->at(i1)) == 13)
				{
					h_den_vs_etamu->Fill((*eta)[i1]);
					h_den_vs_phimu->Fill((*phi)[i1]);
					h_den_vs_ptmu->Fill((*pt)[i1]);
				}
				//else if(match_flagTTT_bc != 1 && (*pt)[i1]>30e3)
				//{
				//	std::cout<<"pdg_id of tracks not reconstructed: " <<  pdg->at(i1) <<std::endl;
				//	std::cout<<"type of interaction: " << (*tinteraction)[i1] <<std::endl;
				//
				//}
